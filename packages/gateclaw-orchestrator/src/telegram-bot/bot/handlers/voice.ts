...
function getTelegramDownloadAgent(): https.RequestOptions["agent"] | undefined {
  ...
  telegramDownloadAgent = proxyUrl.startsWith("socks") ? new SocksProxyAgent(proxyUrl) : new HttpsProxyAgent(proxyUrl)
...
export interface VoiceMessageDeps extends ProcessPromptDeps {
  ...
  downloadTelegramFile?: (ctx: Context, fileId: string) => Promise<{ buffer: Buffer; filename: string } | null>
...
export async function handleVoiceMessage(ctx: Context, deps: VoiceMessageDeps): Promise<void> {
  ...
  try {
    ...
    const recognizedText = await transcribe(fileData.buffer, fileData.filename)
