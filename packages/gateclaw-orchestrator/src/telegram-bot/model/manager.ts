...
function filterModelsByCatalog(models: FavoriteModel[], validModelKeys: Set<string> | null): FavoriteModel[] {
  if (!validModelKeys) {
    return models
  }

  logger.debug(
    `[ModelManager] filterModelsByCatalog: validModelKeys=${JSON.stringify([...validModelKeys].slice(0, 10))}..., inputModels=${JSON.stringify(models.map((m) => getModelKey(m.providerID, m.modelID)))}`,
  )

  const filtered = models.filter((model) => validModelKeys.has(getModelKey(model.providerID, model.modelID)))

  if (filtered.length < models.length) {
    const removedKeys = models
      .filter((m) => !validModelKeys.has(getModelKey(m.providerID, m.modelID)))
      .map((m) => getModelKey(m.providerID, m.modelID))
    logger.debug(`[ModelManager] filterModelsByCatalog: removed=${JSON.stringify(removedKeys)}`)
  }
...
async function getValidModelKeys(): Promise<Set<string> | null> {
  ...
  modelCatalogFetchInFlight = (async () => {
    try {
      logger.debug("[ModelManager] Refreshing model catalog from GateClaw daemon API")

      // Use daemon's /provider endpoint (port 7371) which has custom providers from gateclaw.jsonc
      // Not OpenCode's /provider (port 4100) which only has default providers
      const response = await fetch("http://localhost:7371/provider", { signal: AbortSignal.timeout(5000) })

      if (!response.ok) {
        logger.warn("[ModelManager] Failed to fetch providers from daemon:", response.status)
        return cachedValidModelKeys
      }

      const data = (await response.json()) as { data?: Array<{ id: string; models?: Record<string, unknown> }> }
      const providers = data?.data

      if (!providers || !Array.isArray(providers)) {
        logger.warn("[ModelManager] Invalid provider response from daemon")
        return cachedValidModelKeys
      }
      ...
      logger.debug(
        `[ModelManager] Model catalog refreshed: providers=${providers.length}, models=${validModelKeys.size}`,
      )
