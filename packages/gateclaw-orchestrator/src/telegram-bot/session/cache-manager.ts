...
const SERVER_UNAVAILABLE_ERROR_MARKERS = ["fetch failed", "econnrefused", "connection refused", "connect refused"]
...
async function querySessionDirectoriesFromSqlite(dbPath: string): Promise<CachedSessionDirectory[] | null> {
  try {
...
async function ingestFromGlobalSessionStorage(): Promise<void> {
  ...
  for (const storageRoot of candidates) {
    ...
    try {
      ...
      const sorted = withMtime.sort((a, b) => b.mtimeMs - a.mtimeMs).slice(0, STORAGE_FALLBACK_SCAN_LIMIT)
...
export async function upsertSessionDirectory(worktree: string, lastUpdated: number = Date.now()): Promise<void> {
  await ensureCacheLoaded()
